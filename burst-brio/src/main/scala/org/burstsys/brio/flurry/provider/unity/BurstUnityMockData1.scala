...
/**
  ...
  */
final case
class BurstUnityMockData1(itemCount: Int = 1000, sessionCount:Int = 10, eventCount:Int = 10) extends BrioMockDataModel {
  ...
  lazy val items: Seq[UnityMockInstance] =
    for (i <- 0 until itemCount) yield {
      UnityMockUser(
        ...
        sessions = (for (i <- 0 until sessionCount) yield (mockSession)).toArray,
        interests = Array(
          x = 1L
        ),
        traits = Array(
  ...
  private def mockSession: UnityMockSession = {
    UnityMockSession(
      id = 1L,
      events = (for (i <- 0 until eventCount) yield (mockEvent)).toArray,
      variants = Array(
