...
object BrioLongStringMapCodec {

  @inline final
  def calcLongStringMapSize(count: Int): TeslaMemorySize =
    SizeOfCount + (count * (SizeOfLong + SizeOfString))
  ...
  def writeLongStringMap(data: Map[Long, BrioDictionaryKey], writer: TeslaBufferWriter): Unit = {
    ...
    sorted.foreach(d => writer.writeLong(d._1)) // keys
   ...
   */
  @inline final
  def lookupLongStringMap(mKey: Long, reader: TeslaBufferReader, offset: TeslaMemoryOffset): BrioDictionaryKey = {
    var cursor: TeslaMemoryOffset = offset
    val length = reader.readShort(cursor).toInt
    if (length == 0) return BrioDictionaryNotFound
    cursor += SizeOfCount
    binarySearchLong(reader, cursor, length, mKey).toShort
  }
   ...
   */
  @inline final
  def lookupLongStringMapIsNull(mKey: Long, reader: TeslaBufferReader, offset: TeslaMemoryOffset): Boolean = {
    var cursor: TeslaMemoryOffset = offset
    val length: Int = reader.readShort(cursor).toInt
    if (length == 0) return true
    cursor += SizeOfCount.toInt
    binarySearchLong(reader, cursor, length, mKey) == BrioDictionaryNotFound.toInt
  }

  @inline final
  def lookupLongStringMapSize(reader: TeslaBufferReader, offset: TeslaMemoryOffset): BrioCount = {
    val cursor: TeslaMemoryOffset = offset
    reader.readShort(cursor)
  }

  @inline final
  def lookupLongStringMapKeys(reader: TeslaBufferReader, offset: TeslaMemoryOffset): Array[Long] = {
    var cursor: TeslaMemoryOffset = offset
    ...
    var i = 0
    while (i < length) {
      result(i) = reader.readLong(keyStart + (i * SizeOfLong))
      i += 1
    }
    result
  }
   ...
   */
  @inline private
  def binarySearchLong(reader: TeslaBufferReader, offset: TeslaMemoryOffset, length: Int, key: Long): Int = {
    // start of the values list
    val valuesOffset: Int = offset.toInt + (length * SizeOfLong.toInt)
  ...
  @inline private
  def keyValue(reader: TeslaBufferReader, offset: TeslaMemoryOffset, keyIndex: TeslaMemoryOffset): Long =
    reader.readLong(offset + (keyIndex * SizeOfLong))
