...
import static org.burstsys.motif.symbols.functions.Functions.SPLIT;
import static java.lang.String.format;

/**
 * Split function is a dimension that does takes a relation field and compares it to a sequence of values
 */
public class SplitFunction extends FunctionDefinitionContext {
    ...
    public DataType getDtype(UsageContext context) {
        return ((SplitContext)context).target.getDtype();
    }

    // call context information for a given call instance
    public static class SplitContext implements FunctionExpression.FunctionContext {
       @Override
        public String getFunctionName() {
            return SPLIT;
        }
        public List<ValueExpression> boundaries;
    ...
    public void validate(PathSymbols symbols, FunctionExpression call, FunctionExpression.FunctionContext context, Path scope, Stack<Evaluation> stack) {
        SplitContext fc = (SplitContext)call.getContext();

        if (call.getParms().size() < 2)
            throw new ParseException(call.getLocation(), format("%s requires a relation and one or more range values", this.getName()));
        ...
        fc.boundaries = new ArrayList<>(call.getParms().size()-1);
    ...
    @Override
    public SplitContext bind(PathSymbols pathSymbols, List<ValueExpression> parms, Stack<Evaluation> stack) {
