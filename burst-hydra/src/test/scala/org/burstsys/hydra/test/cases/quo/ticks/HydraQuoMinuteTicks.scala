...
object HydraQuoMinuteTicks extends HydraUseCase(1, 1, "quo") {
  ...
  override val frameSource: String =
    s"""
     ...
     |     cube user.sessions {
     |       dimensions {
     |         'minuteTicks':verbatim[long]
     |       }
     |     }
     |   }
     |   user.sessions => {
     |     pre => {
     |       $analysisName.$frameName.'minuteTicks' = minuteTicks(3)
  ...
  val expected: Array[Any] = Array(180000)
