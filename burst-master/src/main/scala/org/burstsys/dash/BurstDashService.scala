...
object BurstDashService {
  ...
  def apply(
             ...
             master: FabricMasterContainer,
             torcher: BurstDashTorcherService
           ): BurstDashService = RestServiceContext(modality, agent, catalog, master, torcher)
...
class RestServiceContext(
  ...
  def startWebsocketServices(): Unit = {
    fabricRelay = BurstRestFabricTopologyRelay(master.topology, webSocketService)
    master.topology talksTo fabricRelay
