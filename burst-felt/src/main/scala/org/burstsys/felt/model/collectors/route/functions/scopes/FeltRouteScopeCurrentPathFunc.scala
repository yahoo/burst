...
object FeltRouteScopeCurrentPathFunc {
  final val functionName: String = "routeScopeCurrentPath"
}

/**
 * the current (dirty) path last recorded in the journal for this scope
 */
trait FeltRouteScopeCurrentPathFunc extends FeltRouteScopeFunction {

  final override val nodeName = "felt-route-scope-current-path-func"

  final val functionName: String = FeltRouteScopeCurrentPathFunc.functionName

  final override val usage: String =
    s"""
       |usage: $functionName(route) -> integer
       |    the current (dirty) path last recorded in the journal for this scope""".stripMargin
