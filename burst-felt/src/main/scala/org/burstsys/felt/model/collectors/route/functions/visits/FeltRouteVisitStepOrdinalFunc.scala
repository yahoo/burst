...
object FeltRouteVisitStepOrdinalFunc {
  final val functionName: String = "routeVisitStepOrdinal"
}

/**
 * current step ordinal (valid only in a step visit)
 */
trait FeltRouteVisitStepOrdinalFunc extends FeltRouteVisitFunction {

  final override val nodeName = "felt-route-visit-step-ordinal-func"

  final val functionName: String = FeltRouteVisitStepOrdinalFunc.functionName

  final override val usage: String =
    s"""
       |usage: $functionName(route) -> long
       |  current step ordinal (valid only in a step visit)""".stripMargin
  ...
  def generateExpression(implicit cursor: FeltCodeCursor): FeltCode = {
    ...
    s"""|
        |${T(this)}
        |$I${cursor.callScope.scopeNull} = false; ${cursor.callScope.scopeVal} = $visitInstance.stepOrdinal; """.stripMargin
  }
