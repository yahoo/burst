...
object FeltRouteLastStepTagFunc {
  final val functionName: String = "routeLastStepTag"
}

/**
 * returns a count of the full paths recorded in the route
 */
trait FeltRouteLastStepTagFunc extends FeltRouteCommonFunction {

  final override val nodeName = "felt-route-last-step-tag-func"

  final val functionName: String = FeltRouteLastStepTagFunc.functionName

  final override val usage: String =
    s"""
       |usage: $functionName(route) -> long
       |  returns the step tag for the last (committed) step recorded in the route (null if route is empty)""".stripMargin
