...
object FeltRouteVisitStepKeyFunc {
  final val functionName: String = "routeVisitStepKey"
}

/**
 * current step ordinal (valid only in a step visit)
 */
trait FeltRouteVisitStepKeyFunc extends FeltRouteVisitFunction {

  final override val nodeName = "felt-route-visit-step-key-func"

  final val functionName: String = FeltRouteVisitStepKeyFunc.functionName

  final override val usage: String =
    s"""
       |usage: $functionName(route) -> long
       |  current step key (valid only in a step visit)""".stripMargin
  ...
  def generateExpression(implicit cursor: FeltCodeCursor): FeltCode = {
    ...
    s"""|
        |${T(this)}
        |$I${cursor.callScope.scopeNull} = false; ${cursor.callScope.scopeVal} = $visitInstance.stepKey; """.stripMargin
  }
