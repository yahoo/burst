...
/**
 * A literal value expression that reduces to a value array within the felt model.
 ...
 */
trait FeltValArrLit extends FeltMutableLit[FeltExpression] {

  final override val nodeName = "felt-val-array-lit"
  ...
  final override
  def reduceStatics: FeltLiteral = new FeltValArrLit {
    sync(FeltValArrLit.this)
    final override val members: Array[FeltExpression] = FeltValArrLit.this.members.map(_.reduceStatics.resolveTypes)
    final override val location: FeltLocation = FeltValArrLit.this.location
  }
  ...
  final override
  def normalizedSource(implicit index: Int): String = s"array(${members.map(_.normalizedSource).mkString(", ")})"
