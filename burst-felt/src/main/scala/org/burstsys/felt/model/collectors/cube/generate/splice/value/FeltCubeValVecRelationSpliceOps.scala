...
/**
 * We are visiting a value vector that is in a __1:n__ relation within its parent object.
 * <ul>
 * <li>'''FeltVectorAllocPlace:'''  allocate a new ''currentRelationCube'' to hold the overall value vector iterated visit</li>
 * <li>'''FeltChildMergePlace:'''  for each member of the map, after each visit, __merge__  the current value vector into the overall ''currentRelationCube''
 * (a join __will not__ happen)</li>
 * <li>'''FeltChildJoinPlace:'''  for each member of the map, after each visit, __join__  the current value vector into the overall ''currentRelationCube''
 * (a merge __will not__ happen)</li>
 * </ul>
 * '''NOTE:''' we do not allocate a cube per value in the vector for now - we may need to later
 */
private[splice]
trait FeltCubeValVecRelationSpliceOps extends FeltSweepGenerator {

  /**
   * FeltVectorAllocPlace
   */
  final
  def cubeValVecAllocSplice(implicit cube: FeltCubeDecl): FeltSpliceGenerator =
    implicit cursor => {
   ...
   */
  final
  def cubeValVecRelMergeSplice(implicit cube: FeltCubeDecl): FeltSpliceGenerator =
    implicit cursor => {
   ...
   */
  final
  def cubeValVecRelJoinSplice(implicit cube: FeltCubeDecl): FeltSpliceGenerator =
    implicit cursor => {
