...
object FeltRouteScopeCommitFunc {
  final val functionName: String = "routeScopeCommit"
}

/**
 * commit a route transaction  (roll forward clean ptr to dirty ptr)
 */
trait FeltRouteScopeCommitFunc extends FeltRouteScopeFunction {

  final override val nodeName = "felt-route-scope-commit-func"

  final val functionName: String = FeltRouteScopeCommitFunc.functionName

  final override val usage: String =
    s"""
       |usage: $functionName(route) -> unit
       | commit a route transaction  (roll forward clean ptr to dirty ptr)""".stripMargin
