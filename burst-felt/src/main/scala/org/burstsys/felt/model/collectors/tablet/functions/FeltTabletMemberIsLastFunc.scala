...
object FeltTabletMemberIsLastFunc {
  final val functionName: String = "tabletMemberIsLast"
}

/**
 * add a value to a tablet
 */
trait FeltTabletMemberIsLastFunc extends FeltTabletFunction with FeltBoolExpr {

  final override val nodeName = "felt-tablet-member-is-last-func"

  final val functionName: String = FeltTabletMemberIsLastFunc.functionName

  final override val usage: String =
    s"""
       |usage:  $functionName(tablet_name) -> unit
       |    is this the last member of the tablet? (value only during a visit)""".stripMargin
  ...
  def generateExpression(implicit cursor: FeltCodeCursor): FeltCode = {
    ...
    val variable = s"$sweepRuntimeSym.${tablet.memberIsLastVariable}"
    s"""|
        |${T(this)}
