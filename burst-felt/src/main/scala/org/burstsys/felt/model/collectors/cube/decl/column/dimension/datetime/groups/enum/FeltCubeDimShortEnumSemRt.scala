 ...
 */
final case
class FeltCubeDimShortEnumSemRt(var values: Array[Short]) extends FeltCubeDimEnumSemRt with GinsuFunctions {
  ...
  override def generateDeclaration(implicit cursor: FeltCodeCursor): FeltCode =
    s"""|
        |$I${classOf[FeltCubeDimShortEnumSemRt].getName}(
        |${generateShortArrayCode(values)(cursor indentRight 1)}
        |$I)""".stripMargin
  ...
  def doShort(v: Short)(implicit threadRuntime: BrioThreadRuntime): BrioPrimitive = {
    brioShortToPrimitive(shortEnumSlice(values, v))
  }
  ...
  def read(kryo: Kryo, in: Input): Unit = {
    super.read(kryo, in)
    values = kryo.readClassAndObject(in).asInstanceOf[Array[Short]]
  }
