...
object FeltRouteLastStepTimeFunc {
  final val functionName: String = "routeLastStepTime"
}

/**
 * returns a count of the full paths recorded in the route
 */
trait FeltRouteLastStepTimeFunc extends FeltRouteCommonFunction {

  final override val nodeName = "felt-route-last-step-time-func"

  final val functionName: String = FeltRouteLastStepTimeFunc.functionName

  final override val usage: String =
    s"""
       |usage: $functionName(route) -> long
       |  returns the step time for the last (committed) step recorded in the route (null if route is empty)""".stripMargin
