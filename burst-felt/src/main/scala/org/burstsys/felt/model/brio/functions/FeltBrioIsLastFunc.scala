...
object FeltBrioIsLastFunc {
  final val functionName: String = "isLast"
}

/**
 *
 */
trait FeltBrioIsLastFunc extends FeltBrioFunction with FeltBoolExpr {

  final override val nodeName = "felt-brio-is-last-func"
  ...
  def generateExpression(implicit cursor: FeltCodeCursor): FeltCode = {
    ...
    s"""|
        |$I{ // ${M(this)}
        |$I2${cursor.callScope.scopeVal} = ${latticeVectorIsLastViaSweepRuntime(decl.brioNode.pathName)};
