...
object FeltRouteLastStepOrdinalFunc {
  final val functionName: String = "routeLastStepOrdinal"
}
 ...
 */
trait FeltRouteLastStepOrdinalFunc extends FeltRouteCommonFunction {

  final override val nodeName = "felt-route-last-step-ordinal-func"

  final val functionName: String = FeltRouteLastStepOrdinalFunc.functionName

  final override val usage: String =
    s"""
       |usage: $functionName(route) -> long
       |  returns the step ordinal for the last (committed) step recorded in the route (null if route is empty)""".stripMargin
