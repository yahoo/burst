 ...
 * for scans
 */
trait FabricTopologyTender extends AnyRef with FabricSupervisorListener {

  self: FabricSupervisorTopology =>
  ...
  private def addWorker(worker: FabricWorkerProxy): Unit = {
    lazy val tag = s"FabricTopologyTender.addWorker(${worker.link})"
    if (debugTopology)
      log info tag
  ...
  private def lostWorker(worker: FabricWorkerProxy): Unit = {
    lazy val tag = s"FabricTopologyTender.lostWorker(${worker.link})"
    if (debugTopology)
      log info tag
    val key = worker.forExport
