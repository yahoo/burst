...
package object store extends VitalsLogger {
   ...
   */
  final
  def startWorkerStores(container: FabricWorkerContainer): Unit = {
    synchronized {
      if (_loaded) return
      storeProviders.foreach {
        provider =>
          try {
            ...
            val constructor = try {
              provider.workerClass.getConstructor(classOf[FabricWorkerContainer])
            } catch safely {
              case t:Throwable =>
  ...
  final
  def stopWorkerStores(container: FabricWorkerContainer): Unit = {
    synchronized {
