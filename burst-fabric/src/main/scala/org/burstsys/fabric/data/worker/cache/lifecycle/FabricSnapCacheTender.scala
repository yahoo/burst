...
trait FabricSnapCacheTender extends AnyRef {
  ...
  final protected class TenderBackground extends VitalsBackgroundFunction(
    "fab-snap-tender", limits.tendStartWait, limits.tendPeriod, {
      ...
      try {
        if (limits.memoryUsageAboveHighWater) evictConstrainedSnaps()
        if (limits.diskUsageAboveHighWater) flushConstrainedSnaps()
        evictStaleSnaps()
        flushStaleSnaps()
        eraseStaleSnaps()
      } catch safely {
        case t: Throwable => log error burstStdMsg(s"CACHE_TEND_FAIL $t", t)
      }
