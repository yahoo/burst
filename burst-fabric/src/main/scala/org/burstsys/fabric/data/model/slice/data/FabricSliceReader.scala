...
import org.burstsys.fabric.data.model.slice.state.{FabricDataHot, FabricDataNoData, FabricDataWarm, FabricDataCold}
...
trait FabricSliceReader extends AnyRef {
  ...
  def evictSliceFromMemory(): Unit = {
    ...
    if(!sliceInMemory) log.warn(s"NOT_IN_MEMORY! $tag")
    synchronized {
      try {
