...
import org.burstsys.fabric.data.model.snap.{ColdSnap, FabricSnap, FailedSnap, HotSnap, NoDataSnap, WarmSnap}
...
trait FabricSnapCacheCleaner extends AnyRef {
  ...
  def doEvict(snap: FabricSnap): Boolean = {
    ...
    try {
      snap.state match {
        case ColdSnap => log warn s"FAB_CACHE_EVICT_COLD_SNAP $tag "
        case WarmSnap => log warn s"FAB_CACHE_EVICT_WARM_SNAP $tag "
        case HotSnap =>
          ...
          return true
        case NoDataSnap => log warn s"FAB_CACHE_EVICT_NO_DATA_SNAP $tag "
        case FailedSnap => log warn s"FAB_CACHE_EVICT_FAILED_SNAP $tag "
        case s => log warn s"FAB_CACHE_EVICT_UNKNOWN_SNAP state=$s $tag "
      }
      false
    } catch safely {
      case t: Throwable =>
  ...
  def doFlush(snap: FabricSnap): Boolean = {
    ...
    try {
      snap.state match {
        case ColdSnap => log warn s"FAB_CACHE_FLUSH_COLD_SNAP $tag "
        case WarmSnap =>
          ...
          return true
        case HotSnap => log warn s"FAB_CACHE_FLUSH_HOT_SNAP $tag "
        case NoDataSnap =>
          ...
          talk(_.onSnapFlush(snap, System.nanoTime - flushStart, snap.metadata.generationMetrics.byteCount))
          return true
        case FailedSnap => log warn s"FAB_CACHE_FLUSH_FAILED_SNAP $tag "
        case s => log warn s"FAB_CACHE_FLUSH_UNKNOWN_SNAP state=$s $tag "
      }
      false
    } catch safely {
      case t: Throwable =>
