...
package org.burstsys.fabric.execution.master.wave.request

import org.burstsys.fabric.execution.master.FabricScatteredGatherRequest
...
import org.burstsys.fabric.trek.FabricMasterParticleTrekMark
...
class FabricParticleRequest(worker: FabricWorkerProxy, particle: FabricParticle) extends FabricScatteredGatherRequest {
  ...
  /**
   * send a message from the master to a single worker/slice (particle). This is async
  ...
  def execute: Future[Unit] = {
    ...
    slot.slotBegin()
    FabricMasterParticleTrekMark.begin(guid)
    worker.connection.executeParticle(slot, particle) onComplete {
      case Success(r) =>
        promise.success((): Unit)
        FabricMasterParticleTrekMark.end(guid)
        result = r
        ...
        if (resultsWillNotBeProcessed) {
          ...
          // result.releaseResourcesOnMaster()
        } else {
          slot.slotSuccess()
        }
      ...
      case Failure(t) =>
        ...
        promise.failure(t)
        FabricMasterParticleTrekMark.fail(guid)
        if (slot != null) {
          slot.slotFailed(t)
        }
    }
