...
import org.burstsys.fabric.execution.supervisor.FabricSupervisorExecutionContext
import org.burstsys.fabric.execution.supervisor.wave.request.FabricParticleRequest
...
trait FabricWaveScatter extends Any {
  ...
  self: FabricSupervisorExecutionContext =>
    ...
    wave.particles foreach {
      particle =>
        container.topology.getWorker(particle.slice.worker) match {
          case Some(worker) =>
            scatter.addRequestSlot(FabricParticleRequest(worker, particle))
          case None =>
