...
/**
  * handle events from a remote fabric protocol client-worker to the local fabric protocol server-master
...
trait FabricNetServerListener extends Any {
  ...
  /**
    * tell a fabric server-master that fabric client-worker has ''tethered'' (heartbeat after connection)
  ...
  /**
    * tell a fabric server-master that fabric client-worker has ''disconnected'' (presumably died or partitioned)
    * (this is on a fabric protocol [[TeslaRequestThreadPool]])
    */
  def onNetServerDisconnect(connection: FabricNetServerConnection): Unit = {}

  /**
    * tell a fabric server-master that fabric client-worker cache operation request is delivering a response
    * (this is on a fabric protocol [[TeslaRequestThreadPool]])
    */
  def onNetServerCacheOperationRespMsg(connection: FabricNetServerConnection, msg: FabricNetCacheOperationRespMsg): Unit = {}

  /**
    * tell a fabric server-master that fabric client-worker slice information request is delivering a response
    * (this is on a fabric protocol [[TeslaRequestThreadPool]])
    */
  def onNetServerSliceFetchRespMsg(connection: FabricNetServerConnection, msg: FabricNetSliceFetchRespMsg): Unit = {}

  /**
    * tell a fabric server-master that fabric client-worker cache assessment request is delivering a response
    * (this is on a fabric protocol [[TeslaRequestThreadPool]])
    */
  def onNetServerAssessRespMsg(connection: FabricNetServerConnection, msg: FabricNetAssessRespMsg): Unit = {}

  /**
   * tell a fabric server-master that fabric client-worker cache particle request is delivering a response
   * (this is on a fabric protocol [[TeslaRequestThreadPool]])
   */
