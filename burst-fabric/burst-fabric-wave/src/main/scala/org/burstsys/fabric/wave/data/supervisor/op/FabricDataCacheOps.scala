...
import org.burstsys.fabric.wave.data.model.generation.{FabricGeneration, FabricGenerationIdentity}
...
import org.burstsys.fabric.wave.data.supervisor.FabricWaveSupervisorDataContext
...
/**
 * the cache operational aspect of the supervisor side data service
 */
...
class CacheManageOperation(
                            data: FabricWaveSupervisorDataContext,
  ...
  def execute: Future[Unit] = {
    ...
    data.container.topology.getWorker(worker) match {
      ...
      case Some(w) =>
        data.container.cacheManageOperation(w.connection, guid, ruid, operation, generationKey) onComplete {
          case Success(r) =>
  ...
  def execute: Future[Unit] = {
    ...
    data.container.topology.getWorker(worker) match {
      case None =>
        promise.failure(VitalsException(s"$tag $worker not found").fillInStackTrace())
      case Some(w) =>
        data.container.cacheSliceFetchOperation(w.connection, guid, ruid, generationKey) onComplete {
          case Success(r) =>
            promise.success((): Unit)
            result = r.toIndexedSeq
            if (slot != null)
