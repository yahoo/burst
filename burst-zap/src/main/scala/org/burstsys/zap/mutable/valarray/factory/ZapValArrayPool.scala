...
import org.burstsys.zap.mutable.valarray.{ZapValArr, ZapValArrayAnyVal, ZapValArrayBuilder}
 ...
 */
final
case class ZapValArrayPool(poolId: TeslaPoolId, partByteSize: TeslaMemoryOffset)
  extends TeslaPartPool[ZapValArr] with ZapValArrayShop {
  ...
  def grabValArray(builder: ZapValArrayBuilder, startSize: TeslaMemorySize = teslaBuilderUseDefaultSize): ZapValArr = {
    ...
    val part = partQueue poll match {
      case null =>
        ...
        val cube = ZapValArrayAnyVal(tesla.block.factory.grabBlock(size).blockBasePtr)
  ...
  @inline override
  def releaseValArray(d: ZapValArr): Unit = {
    decrementPartsInUse()
    partQueue add d
  }

  @inline override
  def freePart(part: ZapValArr): Long = {
