...
package object flex extends VitalsLogger {
  ...
  final val defaultStartSize: Int = 1e4.toInt // 10K byte start size as a default (can be overridden)
  ...
  final def releaseFlexCube(cube: ZapCube2): Unit = {
    coupler.freeFlexCollectorProxy(cube.asInstanceOf[ZapFlexCube2])
  }
  ...
  lazy val coupler: TeslaFlexCoupler[ZapCube2Builder, ZapCube2, ZapFlexCube2] =
    new TeslaFlexCoupler[ZapCube2Builder, ZapCube2, ZapFlexCube2] {
      ...
      val collectorName: String = "flex_cube2"

      final override val powersOf2SlotCount: TeslaMemorySize = 18 // 256K max cubes
